...
def get_batch(data_instances:List[Data], pad_token_id:int, device:str):
    ...
    # intialize tensors
    input_ids = torch.ones((len(data_instances), max_seq_len), dtype=torch.long) * pad_token_id
    if not is_sequence_transformer:
        relative_position = torch.zeros((len(data_instances), max_seq_len, max_seq_len), dtype=torch.long)
        sparsity_mask = torch.zeros((len(data_instances), max_seq_len, max_seq_len), dtype=torch.bool)
        use_additional_bucket = torch.zeros((len(data_instances), max_seq_len, max_seq_len), dtype=torch.bool)
    ...
    label = torch.tensor([data.label for data in data_instances])
...
#    model.to(args.device)
    ...
    if not args.reload_data:
        ...
        data = {split: [data_to_dataT5(graph, model.tokenizer, label, label_to_index, args.graph_representation, eos=args.eos_usage) for graph, label in tqdm(zip(graphs[split], labels[split]), total=len(labels[split]))] for split in ['test']}
    ...
    for epoch in range(args.num_epochs):
        ...
        train_loss, train_accuracy = run_train_epoch(model=model, data=data['test'], criterion=criterion, optimizer=optimizer, batch_size=args.train_batch_size, gradient_accumulation_steps=args.gradient_accumulation_steps, device=args.device)
