        ...
        for(OWLSubObjectPropertyOfAxiom subPropertyAxiom : subPropertyAxiomSet) {
            ...
            for (OWLClassExpression c1 : subClassOfAxiomSet.stream().map(OWLSubClassOfAxiom::getSubClass).collect(Collectors.toSet())){
                Set<OWLObjectAllValuesFrom> superClasses = subClassOfAxiomSet.stream()
                        ...
                        .collect(Collectors.toSet());
                Set<OWLClassExpression> possibleC2 = superClasses.stream().filter(ax -> ax.getProperty().equals(r1)).map(OWLObjectAllValuesFrom::getFiller).collect(Collectors.toSet());
                Set<OWLClassExpression> possibleC3 = superClasses.stream().filter(ax -> ax.getProperty().equals(r2)).map(OWLObjectAllValuesFrom::getFiller).collect(Collectors.toSet());
