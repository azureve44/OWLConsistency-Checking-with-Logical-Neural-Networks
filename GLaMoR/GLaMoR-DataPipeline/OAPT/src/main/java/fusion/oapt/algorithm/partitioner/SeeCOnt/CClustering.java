			   ...
			   
	   {
	    ...
	    switch (arg){
	   	case 0: //call from Execute button to partition the whole ontology (normal case)
	   		...
	   		part.partition();
	   		clusters=part.getPartitions();
                        logClusterSummary("EXECUTE", clusters);
	   		m=new CreateModule(MB,clusters);
	   		m.createOWLFiles_Phase(); 
	   		modules=m.getOntModels();
                        logModuleSummary("EXECUTE", modules);
	   		break;
		...
		case 1: //call from Apply button of adding CH (as an interactive process)
			...
			clusters=part.getPartitions();
                        logClusterSummary("INTERACTIVE", clusters);
			 m=new CreateModule(MB,clusters);
		   	 m.createModules();
			 m.createOWLFiles_Phase();
			 modules=m.getOntModels();
                        logModuleSummary("INTERACTIVE", modules);
			break;
...
//	   		m.createOutput_old_Phase();
   ...
   private void logClusterSummary(String stage, LinkedHashMap<Integer, Cluster> currentClusters)
   {
           int chosenK = NoCH > 0 ? NoCH : currentClusters != null ? currentClusters.size() : 0;
           int clusterCount = currentClusters != null ? currentClusters.size() : 0;
           System.out.println("[OAPT-DIAG|CLUSTER_RESULT] ontology=" + ontName
                           + " stage=" + stage
                           + " chosenK=" + chosenK
                           + " clusterCount=" + clusterCount
                           + " clusterSizes=" + summarizeClusterSizes(currentClusters));
           if (chosenK > 1 && clusterCount == 1) {
                   System.out.println("[OAPT-WARN|CLUSTER_COLLAPSE] ontology=" + ontName
                                   + " stage=" + stage
                                   + " chosenK=" + chosenK
                                   + " clusterCount=" + clusterCount);
           }
   ...
   private void logModuleSummary(String stage, ArrayList<OntModel> currentModules)
   {
           int clusterCount = clusters != null ? clusters.size() : 0;
           int moduleCount = currentModules != null ? currentModules.size() : 0;
           System.out.println("[OAPT-DIAG|MODULE_RESULT] ontology=" + ontName
                           + " stage=" + stage
                           + " clusterCount=" + clusterCount
                           + " moduleCount=" + moduleCount
                           + " moduleNamedClassCounts=" + summarizeModuleSizes(currentModules));
           if (clusterCount > 1 && moduleCount == 1) {
                   System.out.println("[OAPT-WARN|MODULE_COLLAPSE] ontology=" + ontName
                                   + " stage=" + stage
                                   + " clusterCount=" + clusterCount
                                   + " moduleCount=" + moduleCount);
           }
   ...
   private static String summarizeClusterSizes(LinkedHashMap<Integer, Cluster> currentClusters)
   {
           ...
           int index = 0;
           for (Cluster cluster : currentClusters.values()) {
                   if (index > 0) sb.append(", ");
                   sb.append(cluster.getClusterID()).append(':').append(cluster.getSize());
                   index++;
                   if (index >= 8 && currentClusters.size() > index) {
                           sb.append(", ... total=").append(currentClusters.size());
                           break;
                   }
           }
           sb.append(']');
   ...
   private static String summarizeModuleSizes(ArrayList<OntModel> currentModules)
   {
           if (currentModules == null || currentModules.isEmpty()) {
                   return "[]";
           }
           StringBuilder sb = new StringBuilder("[");
           int limit = Math.min(currentModules.size(), 8);
           for (int i = 0; i < limit; i++) {
                   if (i > 0) sb.append(", ");
                   OntModel module = currentModules.get(i);
                   sb.append(i).append(':').append(module.listNamedClasses().toList().size());
           }
           if (currentModules.size() > limit) {
                   sb.append(", ... total=").append(currentModules.size());
           }
           sb.append(']');
           return sb.toString();
   }
