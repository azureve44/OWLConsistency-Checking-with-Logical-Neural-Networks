...
public class FindOptimalCluster 
{
	...
	public FindOptimalCluster(String name)
	{
		...
		numEnt=model != null ? model.listNamedClasses().toList().size() : 0;
		entitiesCH=new NodeList();
		lb=ub=enCh=0;
		rankVector=new HashMap<String, Set<String>>();
		nodeRankVector=new HashMap<Node, Set<Node>>();
...
private void logFindKPhase(String phase, long startedAtMillis, String details)
{
	...
	System.out.println("[OAPT-DIAG|FINDK_PROGRESS] ontology=" + nameOnt
											+ " phase=" + phase
											+ elapsedPart
											+ (details != null && details.length() > 0 ? " " + details : ""));
}

private void logFindKPhase(String phase, String details)
{
	logFindKPhase(phase, -1, details);
}
 ...
 public  int FindOptimalClusterFunc()
 {
	...
	if(MB==null)
	{
	 ...
	 if(size<=0) {
		 OptK=0;
		 logSelectionDecision(size, 0, 0, 0, OptK, entitiesCH);
		 return OptK;
	 }
	 if(size<=40) {
		 lb=ub=1;
		 OptK=1;
		 logSelectionDecision(size, lb, ub, 1, OptK, entitiesCH);
		 return OptK;
	 }
	 int d=0; 
	 if(size<1000)
		 d=3;
            ...
            d=0;
         logFindKPhase("before_rank", "entityCount=" + size + " namedClassNodes=" + size + " listNamedClasses=" + size1 + " depthWindow=" + d);
     ...
     logFindKPhase("after_rank", rankStart, "rankedCount=" + (SortedNameOnt != null ? SortedNameOnt.length : 0)
                     + " rankVectorSize=" + (rankVector != null ? rankVector.size() : 0)
                     + " nodeRankVectorSize=" + (nodeRankVector != null ? nodeRankVector.size() : 0));
     NodeList entitiesCHTemp = new NodeList();
     if(SortedNameOnt != null && SortedNameOnt.length>0)
             {
    ...
    //determining the lower and upper bound to know how many model should be create
     ...
     long lowUppStart = System.currentTimeMillis();
     lowupValue = LowUppCalculation();
     lb = lowupValue[0];
     ub = lowupValue[1];
     logFindKPhase("after_low_upp", lowUppStart, "lowerBound=" + lb + " upperBound=" + ub
                     + " initialCandidateCount=" + entitiesCHTemp.size());
     enCh=entitiesCHTemp.size();
     if(enCh<=lb)
    {
             logFindKPhase("skip_check_noop", "initialCandidateCount=" + enCh + " lowerBound=" + lb);
             entitiesCH=entitiesCHTemp;
             OptK=enCh;  Coordinator.KNumCH=OptK;
            logSelectionDecision(size, lb, ub, enCh, OptK, entitiesCH);
            return OptK;
    }
    logFindKPhase("before_check_noop", "candidateCount=" + entitiesCHTemp.size() + " lowerBound=" + lb + " upperBound=" + ub);
    long checkNoOpStart = System.currentTimeMillis();
    int index=checkNoOP(entitiesCHTemp);
    logFindKPhase("after_check_noop", checkNoOpStart, "selectedIndex=" + index + " resultingChosenK=" + (lb + index));
    enCh=lb+index;
    OptK=enCh;  Coordinator.KNumCH=OptK;
    logSelectionDecision(size, lb, ub, entitiesCHTemp.size(), OptK, entitiesCH);
    return OptK;
  }
 private void logSelectionDecision(int entityCount, int lowerBound, int upperBound, int initialCandidateCount, int chosenK, NodeList selectedHeads)
 {
		 System.out.println("[OAPT-DIAG|K_SELECT] ontology=" + nameOnt
						 + " entityCount=" + entityCount
						 + " lowerBound=" + lowerBound
						 + " upperBound=" + upperBound
						 + " initialCandidateCount=" + initialCandidateCount
						 + " chosenK=" + chosenK
						 + " selectedHeadSample=" + summarizeNodeList(selectedHeads, 5));
 }
 private static String summarizeNodeList(NodeList nodes, int maxItems)
 {
		 if (nodes == null || nodes.size() == 0) {
			 return "[]";
		 }
		 StringBuilder sb = new StringBuilder("[");
		 int limit = Math.min(nodes.size(), maxItems);
		 ...
		 if (nodes.size() > limit) {
			 sb.append(", ... total=").append(nodes.size());
		 }
		 sb.append(']');
		 return sb.toString();
 }
