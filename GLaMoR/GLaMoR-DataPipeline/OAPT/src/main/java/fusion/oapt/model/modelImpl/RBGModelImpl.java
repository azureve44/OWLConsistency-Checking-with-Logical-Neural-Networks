...
import fusion.oapt.model.coordination.RuleList;
...
public class RBGModelImpl implements RBGModel
{
    ...
    private  NodeList nodeNamedList;
    public RBGModelImpl()
    {
    ...
    private void logRbgPhase(String phase, long startedAtMillis, String details)
    {
        ...
        System.out.println("[OAPT-DIAG|RBG_PROGRESS] phase=" + phase
                        + elapsedPart
                        + (details != null && details.length() > 0 ? " " + details : ""));
    }
    private void logRbgPhase(String phase, String details)
    {
        logRbgPhase(phase, -1, details);
    }
    public void setOntModel(OntModel model)
    {
        ontModel = model;
        logRbgPhase("setOntModel_enter", "jenaSize=" + (ontModel != null ? ontModel.size() : -1));
        long clearStart = System.currentTimeMillis();
        clearModel();
        logRbgPhase("after_clearModel", clearStart, "nodeSetSize=" + nodeSet.size() + " quadrupleSize=" + quadrupleSet.size());
        long coordinateStart = System.currentTimeMillis();
        coordinate();
        logRbgPhase("after_coordinate", coordinateStart, "statementCount=" + (ontModel != null ? ontModel.size() : -1));
        long transformStart = System.currentTimeMillis();
        transformToRBG();
        logRbgPhase("after_transformToRBG", transformStart, "nodeSetSize=" + nodeSet.size() + " quadrupleSize=" + quadrupleSet.size());
        long layerStart = System.currentTimeMillis();
        layerNodes();
        logRbgPhase("after_layerNodes", layerStart, "nodeSetSize=" + nodeSet.size() + " quadrupleSize=" + quadrupleSet.size());
        if (clrClassType) {
            long clearClassTypeStart = System.currentTimeMillis();
            clearClassType();
            logRbgPhase("after_clearClassType", clearClassTypeStart, "quadrupleSize=" + quadrupleSet.size());
        }
        if (addInstType) {
            long addInstTypeStart = System.currentTimeMillis();
            addInstType();
            logRbgPhase("after_addInstType", addInstTypeStart, "quadrupleSize=" + quadrupleSet.size());
        }
        if (initHier) {
            long hierarchyStart = System.currentTimeMillis();
            constructHierarchy();
            logRbgPhase("after_constructHierarchy", hierarchyStart, "namedNodeCacheSize=" + (nodeNamedList != null ? nodeNamedList.size() : -1));
        }
        logRbgPhase("setOntModel_exit", "nodeSetSize=" + nodeSet.size() + " quadrupleSize=" + quadrupleSet.size());
    }
    ...
    private void coordinate()
    {
        if (rules != null) {
            for (int i = 0; i < rules.size(); i++) {
                ...
                String ruleName = rule != null ? rule.getClass().getSimpleName() : "null";
                logRbgPhase("before_rule", "index=" + i + " rule=" + ruleName + " statementCount=" + (ontModel != null ? ontModel.size() : -1));
                long ruleStart = System.currentTimeMillis();
                rule.coordinate(ontModel);
                logRbgPhase("after_rule", ruleStart, "index=" + i + " rule=" + ruleName + " statementCount=" + (ontModel != null ? ontModel.size() : -1));
            }
            return;
        }
    }
    ...
    {
        logRbgPhase("constructHierarchy_enter",
                        "stmtNodeCount=" + quadrupleSet.size() + " nodeSetSize=" + nodeSet.size());
        long relationPassStart = System.currentTimeMillis();
        int scannedStmtNodes = 0;
        ...
        for (Iterator<Node> iter = listStmtNodes(); iter.hasNext();) {
            ...
            } else if (p.equals(Constant.RDFS_NS + "domain")) {
                Node subject = statement.getSubject();
                Node object = statement.getObject();
                if (!subject.isBuiltIn() && !subject.isAnon() &&
                                !object.isBuiltIn() && !object.isAnon()) {
            ...
            if (scannedStmtNodes % 100000 == 0) {
                logRbgPhase("constructHierarchy_stmt_checkpoint", relationPassStart,
                                "scannedStmtNodes=" + scannedStmtNodes
                                + " hierarchyEdges=" + hierarchyEdges
                                + " domainEdges=" + domainEdges);
            }
        }
        logRbgPhase("constructHierarchy_after_relation_pass", relationPassStart,
                        "scannedStmtNodes=" + scannedStmtNodes
                        + " hierarchyEdges=" + hierarchyEdges
                        + " domainEdges=" + domainEdges);
        long determineStart = System.currentTimeMillis();
        determineHierarchy();
        logRbgPhase("constructHierarchy_exit", determineStart,
                        "namedNodeCacheSize=" + (nodeNamedList != null ? nodeNamedList.size() : -1));
    }
    private void determineHierarchy()
    { /* hierarchy = max(father) + 1 on the SCC condensation DAG */
        ArrayList<Node> resourceNodes = new ArrayList<Node>();
        ...
        long sccStart = System.currentTimeMillis();
        ArrayList<Node> finishOrder = new ArrayList<Node>(resourceNodes.size());
        HashSet<Node> visited = new HashSet<Node>(resourceNodes.size());
        for (int i = 0, n = resourceNodes.size(); i < n; i++) {
            Node node = resourceNodes.get(i);
            if (!visited.contains(node)) {
                collectFinishOrder(node, visited, finishOrder);
            }
        }
        ArrayList<ArrayList<Node>> components = new ArrayList<ArrayList<Node>>();
        HashMap<Node, Integer> componentByNode = new HashMap<Node, Integer>(resourceNodes.size());
        HashSet<Node> assigned = new HashSet<Node>(resourceNodes.size());
        int cyclicComponents = 0;
        int cyclicNodes = 0;
        for (int i = finishOrder.size() - 1; i >= 0; i--) {
            Node node = finishOrder.get(i);
            if (assigned.contains(node)) {
                continue;
            }
            ArrayList<Node> component = collectReverseComponent(node, assigned);
            int componentIndex = components.size();
            components.add(component);
            for (int j = 0, m = component.size(); j < m; j++) {
                componentByNode.put(component.get(j), Integer.valueOf(componentIndex));
            }
            if (isCyclicComponent(component)) {
                cyclicComponents++;
                cyclicNodes += component.size();
            }
        }
        logRbgPhase("determineHierarchy_after_scc", sccStart,
                        "resourceNodes=" + resourceNodes.size()
                        + " components=" + components.size()
                        + " cyclicComponents=" + cyclicComponents
                        + " cyclicNodes=" + cyclicNodes);
        long dagStart = System.currentTimeMillis();
        ArrayList<HashSet<Integer>> componentChildren = new ArrayList<HashSet<Integer>>(components.size());
        for (int i = 0, n = components.size(); i < n; i++) {
            componentChildren.add(new HashSet<Integer>());
        }
        int[] indegree = new int[components.size()];
        int[] componentHierarchy = new int[components.size()];
        Arrays.fill(componentHierarchy, 1);
        for (int i = 0, n = resourceNodes.size(); i < n; i++) {
            Node parent = resourceNodes.get(i);
            Integer parentComponent = componentByNode.get(parent);
            if (parentComponent == null) {
                continue;
            }
            NodeList childList = parent.getNamedSubs();
            if (childList == null) {
                continue;
            }
            for (int j = 0, m = childList.size(); j < m; j++) {
                Node child = childList.get(j);
                Integer childComponent = componentByNode.get(child);
                if (childComponent == null || parentComponent.equals(childComponent)) {
                    continue;
                }
                if (componentChildren.get(parentComponent.intValue()).add(childComponent)) {
                    indegree[childComponent.intValue()]++;
                }
            }
        }
        ArrayDeque<Integer> queue = new ArrayDeque<Integer>();
        for (int i = 0, n = indegree.length; i < n; i++) {
            if (indegree[i] == 0) {
        ...
        int maxHierarchy = 1;
        while (!queue.isEmpty()) {
            int component = queue.removeFirst().intValue();
            processedComponents++;
            int currentHierarchy = componentHierarchy[component];
            if (currentHierarchy > maxHierarchy) {
                maxHierarchy = currentHierarchy;
            }
            Iterator<Integer> children = componentChildren.get(component).iterator();
            while (children.hasNext()) {
                int childComponent = children.next().intValue();
                int candidateHierarchy = currentHierarchy + 1;
                if (componentHierarchy[childComponent] < candidateHierarchy) {
                    componentHierarchy[childComponent] = candidateHierarchy;
                }
                indegree[childComponent]--;
                if (indegree[childComponent] == 0) {
                    queue.addLast(Integer.valueOf(childComponent));
                }
            }
            if (processedComponents % 100000 == 0) {
                logRbgPhase("determineHierarchy_component_checkpoint", dagStart,
        ...
        for (int i = 0, n = components.size(); i < n; i++) {
            int hierarchy = componentHierarchy[i];
            ArrayList<Node> component = components.get(i);
            for (int j = 0, m = component.size(); j < m; j++) {
                component.get(j).setHierarchy(hierarchy);
            }
        }
        logRbgPhase("determineHierarchy_after_dag", dagStart,
                        "processedComponents=" + processedComponents
                        + " totalComponents=" + components.size()
                        + " maxHierarchy=" + maxHierarchy);
    }
    private void collectFinishOrder(Node start, HashSet<Node> visited, ArrayList<Node> finishOrder)
    {
        ArrayDeque<NodeTraversalFrame> stack = new ArrayDeque<NodeTraversalFrame>();
        visited.add(start);
        stack.addLast(new NodeTraversalFrame(start, start.getNamedSubs()));
        while (!stack.isEmpty()) {
            NodeTraversalFrame frame = stack.peekLast();
            if (frame.neighbors != null && frame.index < frame.neighbors.size()) {
                Node next = frame.neighbors.get(frame.index++);
                if (next != null && next.getNodeType() == Node.SPO_RESOURCE && !visited.contains(next)) {
                    visited.add(next);
                    stack.addLast(new NodeTraversalFrame(next, next.getNamedSubs()));
                }
            } else {
    ...
    private ArrayList<Node> collectReverseComponent(Node start, HashSet<Node> assigned)
    {
        ...
        while (!stack.isEmpty()) {
            ...
            component.add(node);
            NodeList parents = node.getNamedSupers();
            if (parents == null) {
                continue;
            }
            for (int i = 0, n = parents.size(); i < n; i++) {
                Node parent = parents.get(i);
                if (parent != null && parent.getNodeType() == Node.SPO_RESOURCE && !assigned.contains(parent)) {
                    assigned.add(parent);
    ...
    private boolean isCyclicComponent(ArrayList<Node> component)
    {
        if (component.size() > 1) {
            return true;
        }
        if (component.size() == 1) {
            Node node = component.get(0);
            NodeList children = node.getNamedSubs();
            if (children != null) {
    ...
    private static final class NodeTraversalFrame
    {
        private final Node node;
        private final NodeList neighbors;
        private int index = 0;
        private NodeTraversalFrame(Node node, NodeList neighbors)
        {
            this.node = node;
    ...
    public void setNamedClassNodes(NodeList nodes)
    {
    	this.nodeNamedList=nodes;
    }
    
    public NodeList getnamedNodes()
    {
    	if(nodeNamedList==null)
    		return getNamedClassNodes();
    	return nodeNamedList;
    }
    ...
    public NodeList getNamedClassNodes()
    {
        ...
        while (nodeList.hasNext()) {
            ...
            if (!node.isAnon() && 
                ...
                classList.add(node);   nodeNamedList.add(node);
            }
        }
        return classList;
    }
