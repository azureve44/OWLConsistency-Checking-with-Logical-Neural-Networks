...
class Formula(ABC):
    ...
    def get_labels(self, *groundings: str) -> torch.Tensor:
        ...
        data_device = self.get_data(*groundings).device if len(groundings) > 0 else self.get_data().device
        if self.propositional or len(groundings) == 0:
            if isinstance(self.labels, torch.Tensor):
                return self.labels.to(data_device)
            return self.labels
        result = torch.stack([self.labels.get(self._ground(g)) for g in groundings]).to(data_device)
