...
class _NodeActivation(_NodeParameters):
    ...
    def state(self, bounds: torch.Tensor = None) -> np.ndarray:
        ...
        contradictions = self.is_contradiction(args=args)
        if isinstance(contradictions, torch.Tensor):
            contradictions = contradictions.detach().cpu().numpy()
        result = np.where(contradictions, "C", result)
        if result == "0.0":
...
def bool_tensor(func: str, *args: bool) -> torch.BoolTensor:
    """Evaluate boolean tensors on a consistent device."""
    tensor = tensorise(args[0]).to(dtype=torch.int8)
    for a in args[1:]:
        a_tensor = tensorise(a).to(device=tensor.device, dtype=torch.int8)
        if func == "and":
            tensor = tensor * a_tensor
        elif func == "or":
            tensor = tensor + a_tensor
