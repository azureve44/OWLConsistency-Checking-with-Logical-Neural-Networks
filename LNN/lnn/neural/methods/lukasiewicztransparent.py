    ...
    def _and_downward(
        ...
        f_inv = (
            ...
            + (
                ...
                * torch.stack([self.bias - self.weights.sum(), torch.zeros((), device=self.bias.device, dtype=self.bias.dtype)])
            )
            + (
                (operator_bounds >= 1).float()
                * torch.stack([torch.zeros((), device=self.bias.device, dtype=self.bias.dtype), self.bias - 1])
            )
        )
