...
class Model(nn.Module):
    ...
    def to(self, *args, **kwargs):
        """Move model-owned parameters and tensor state to requested device."""
        super().to(*args, **kwargs)
        device = kwargs.get("device", args[0] if len(args) > 0 else None)
        if device is None:
            return self
        device = torch.device(device)
        # Move registered node parameters.
        for param in self.parameters():
            if param.device != device:
                param.data = param.data.to(device)
                if param.grad is not None:
                    param.grad.data = param.grad.data.to(device)
        # Move unregistered tensor state held on symbolic nodes (e.g., bounds tables).
                ...
                if isinstance(value, torch.Tensor) and value.device != device:
                    if isinstance(value, nn.Parameter):
                        ...
                        if value.grad is not None:
                            value.grad.data = value.grad.data.to(device)
                    else:
                        setattr(node, name, value.to(device))
            neuron = getattr(node, "neuron", None)
            if neuron is not None:
                for name, value in vars(neuron).items():
                    if isinstance(value, torch.Tensor) and value.device != device:
                        if isinstance(value, nn.Parameter):
                            value.data = value.data.to(device)
                            if value.grad is not None:
                            ...
                            setattr(neuron, name, value.to(device))
                # Activation thresholds are derived from alpha; rebind after device move.
                if hasattr(neuron, "alpha"):
                    if hasattr(neuron, "Yt"):
                        neuron.Yt = neuron.alpha
                    if hasattr(neuron, "Yf"):
                        neuron.Yf = 1 - neuron.alpha
        return self
    def parameters(self):
        result = list(
